...
def test_validate_auth0_token_valid() -> None:
    """``/auth/auth0/validate`` returns the token subject."""
    token = get_token("admin")
    resp = client.get(
        "/auth/auth0/validate",
        headers={"Authorization": f"Bearer {token}"},
...
def test_validate_auth0_token_invalid() -> None:
    """Invalid tokens return ``403`` from Auth0 endpoint."""
    resp = client.get(
        "/auth/auth0/validate",
